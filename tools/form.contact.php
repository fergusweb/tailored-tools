	...
	class contact_form_log_Table extends tws_form_log_Table {
		...
		function display_rows() {
			...
			foreach ($records as $record) {
				...
				echo '<tr class="more">';
				echo '<td colspan="3">';
				if ($form['cust_message'])	echo 	'<p>'.nl2br(esc_html($form['cust_message'])).'</p>';
				if ($form['Viewing'])		echo 	'<p>Viewing: <a target="_blank" href="'.esc_url($form['Viewing']).'">'.esc_html($form['Viewing']).'</a></p>';
				if (!$form)					echo	"\n<pre>Problem decoding information.\nRaw data: ".esc_html(print_r($record->post_content,true))."</pre>\n";
				echo '</td>';
				echo '</tr>';
			}
		}
